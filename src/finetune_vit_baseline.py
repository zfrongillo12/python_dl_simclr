...
# ViT
from moco.VIT_baseline.model_builder import MoCo

# ViT Wrappers
from moco.VIT_baseline.vit_backbone_wrapper import FT_ViTBackbone, FT_ViT_FinetuneModel
...
def write_cm_to_file(cm, file_path, log_file=None, dataset_title='Pneumonia Classification'):
    ...
    plt.title(f'Confusion Matrix: Finetune ViT {dataset_title}')
...
def freeze_vit_layers(backbone, fraction=0.6):
    ...
    print(f"[freeze] Freezing first {cutoff}/{num_blocks} blocks")
...
def main(args):
    ...
    moco = MoCo(pretrained=False, device=device)
    moco.load_state_dict(ckpt, strict=False)
    ...
    # build normal ViT backbone
    backbone = FT_ViTBackbone(moco.encoder_q).to(device)
    ...
    stats_path = os.path.join(args.artifact_root, 'vit_baseline_finetune_training_stats.pkl')
