...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k=None, return_features=False):
        ...
        q_tokens = self.encoder_q.forward_features(im_q)    # (B, N, D)
        q = q_tokens[:, 0]
        ...
        with torch.no_grad():
            ...
            k_tokens = self.encoder_k.forward_features(im_k)
            k = k_tokens[:, 0]
            k = self.mlp_k(k)
        ...
        # ------------------------------------------------------------------
        # print("DEBUG SHAPES -> q_tokens:", q_tokens.shape, " | q:", q.shape)
        ...
        return logits, labels, q, k   # <-- required return format
